...
RES=$(sbatch -w volta05 --parsable --job-name="dpart_train"  train.sh params.logdir=$depart_logdir params.model=depart "$@" ) && sbatch --dependency=afterok:${RES} --job-name="dpart_eval" evaluation.sh base_logdir=$depart_logdir model=depart
