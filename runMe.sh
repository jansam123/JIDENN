...
# RES=$(sbatch --parsable train.sh params.logdir=$interacting_part_logdir params.model=part part.interaction=False "$@" ) && sbatch --dependency=afterok:${RES} evaluation.sh base_logdir=$interacting_part_logdir model=part 
