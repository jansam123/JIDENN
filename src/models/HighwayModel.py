...
class HighwayModel(tf.keras.Model):
    ...
    def _highway_layers(self, inputs, layer_size: int, num_layers: int, dropout: float | None = None) -> tf.Tensor:
        ...
        for _ in range(num_layers):
            ...
            t = tf.keras.layers.Dense(layer_size, activation=self._gate_activation, bias_initializer=tf.keras.initializers.Constant(-2.0))(hidden)
            hidden = tf.keras.layers.Add()([tf.keras.layers.Multiply()([t, h]), tf.keras.layers.Multiply()([1-t, hidden])])
