...
def main(args):
    ...
    score_dataset = score_dataset.query(
        'jets_pt < 2500000 and jets_pt > 200000 and jets_eta < 2.1 and jets_eta > -2.1')
    ...
    for i, score_name in enumerate(scores):
        if 'score' not in score_name:
            continue
        label = MODEL_NAMING_SCHEMA[score_name.replace('_score', '')] if score_name.replace(
            '_score', '') in MODEL_NAMING_SCHEMA.keys() else score_name.replace('_score', '')
        plot = VarVsEff(
