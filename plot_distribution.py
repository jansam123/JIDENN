...
def plot_single(dataset: tf.data.Dataset, 
    ...
    if log_bins:
        bins = np.logspace(np.log(0.06), np.log(3.9), bins, base=np.e)
    # ylim = (1e2, 1e4)
    ylim = None
    plot_single_dist(df, variable=variable, 
                     hue_var='Truth Label', hue_order=hue_order, 
                     save_path=save_path, bins=bins,
                     ylog=False, xlog=log_bins, xlabel=r'$p_{\mathrm{T}}$ [TeV]',
                     badge_text=badge_text, ylim=ylim
                     )
...
def main(args: argparse.Namespace):
    ...
    if args.plot_single:
        plot_single(dataset.dataset, 
                    ...
                    save_path=args.save_path, badge_text=r'$N_{\mathrm{jets}}$ = ' + f'{ds_size:,} \n' + r'$n_{\mathrm{bins}}$ = '+ f'{args.bins}',
