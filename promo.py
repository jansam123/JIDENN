...
# batch size
BATCH_SIZE = 64
# num of jets//BATCH_SIZE used for testing
TEST_SIZE = 100
# num of batches used to calculate mean and variance for normalization, total jets used for calculation is BATCH_SIZE*num_jets
NORMALIZATION_SIZE = 10
# hidden layers, with given sizes used in model
HIDDEN_LAYERS = [64, 64]
# probability that we will mask a node in hidden layer to avoid overfitting
DROPOUT = 0.3
# number of epochs we want to train, ie. number of times we show the model our entire dataset
EPOCHS = 5

# name of TTree in root file we want to load
TTREE = 'NOMINAL'
# root files that are going to be utilized
FILENAMES = ['/Users/samueljankovych/Documents/MFF/bakalarka/JIDENN/data/data2/user.pleskot.31142731.ANALYSIS._000001.root',
             '/Users/samueljankovych/Documents/MFF/bakalarka/JIDENN/data/data2/JZ04_r10724/user.pleskot.31142736.ANALYSIS._000002.root',
             '/Users/samueljankovych/Documents/MFF/bakalarka/JIDENN/data/data2/JZ04_r10724/user.pleskot.31142736.ANALYSIS._000003.root',
             '/Users/samueljankovych/Documents/MFF/bakalarka/JIDENN/data/data2/JZ04_r10724/user.pleskot.31142736.ANALYSIS._000004.root']
...
# first index is for events, second for jets and third for givern variable per Jet
VARIABLES = ['jets_ActiveArea4vec_eta', 'jets_ActiveArea4vec_m', 'jets_ActiveArea4vec_phi', 'jets_ActiveArea4vec_pt', 'jets_ChargedPFOWidthPt1000[:,:,0]', 'jets_DetectorEta', 'jets_EMFrac', 'jets_EnergyPerSampling[:,:,0]', 'jets_FracSamplingMax', 'jets_FracSamplingMaxIndex', 'jets_GhostMuonSegmentCount', 'jets_JetConstitScaleMomentum_eta', 'jets_JetConstitScaleMomentum_m', 'jets_JetConstitScaleMomentum_phi', 'jets_JetConstitScaleMomentum_pt',
             'jets_JVFCorr', 'jets_Jvt', 'jets_JvtRpt', 'jets_NumChargedPFOPt1000[:,:,0]', 'jets_NumChargedPFOPt500[:,:,0]', 'jets_NumTrkPt1000[:,:,0]', 'jets_NumTrkPt500[:,:,0]', 'jets_SumPtChargedPFOPt500[:,:,0]', 'jets_SumPtTrkPt500[:,:,0]', 'jets_Timing', 'jets_TrackWidthPt1000[:,:,0]', 'jets_Width', 'jets_chf', 'jets_eta', 'jets_fJVT', 'jets_m', 'jets_passFJVT', 'jets_passJVT', 'jets_phi', 'jets_pt']
...
parser.add_argument("--dropout", default=0.1, type=float, help="Size of dropout after each layer.")
...
parser.add_argument("--label", default=LABEL, type=str, help="Name of label we want to predict.")
parser.add_argument("--variables", default=VARIABLES, type=str, nargs="*",
                    help="Name of jet variables used in making prediction.")
...
def main(args: argparse.Namespace):
    # restrict maximum available threads
    tf.config.threading.set_inter_op_parallelism_threads(args.threads)
    tf.config.threading.set_intra_op_parallelism_threads(args.threads)
    # set seed for utilizing random variables
    np.random.seed(args.seed)
    tf.random.set_seed(args.seed)

    # Simplest way to load root files is to dump them in a pandas dataframe.
    ...
    for file in args.root_files[:3]:
        ...
        # by setting library='pd' we can load the directly to pd.DataFrame, but there is bug when TBranches are nested NTuples
    ...
    print(df.head())
    # droping index converts rows indexed by events and jets to be indexed by only by jets
    ...
    # convert pandas dataframes to tensroflow datasets
    # tf.Dataset can't convert pd.DataFrame directly, that is way we convert it to dict first
    ...
    # combine the two datasets with equal probability
    ...
    def prepare_sample(sample: dict[str, tf.Tensor]) -> tuple[tf.Tensor, tf.Tensor]:
        ...
        # this tf.function decorator is another black magic of tensorflow, which allows for faster computation in a Graph Mode
        label = sample[args.label]
        # map original lables [1,2,3,4,5,6,21] into just quarks and just gluons [1,0]
        label = tf.cast(tf.reduce_any(tf.equal(label, args.first_label)), tf.int32)
    ...
    # used the function to map every element
    mixed_dataset = mixed_dataset.map(prepare_sample)
    # group samples to batches
    mixed_dataset = mixed_dataset.batch(args.batch_size)
    # prefetching intructs the dataset to utilize more cores for datq preparation, so the DNN does not wait for data
    ...
    # we need to create the normalization layer before other, becouse we need to calculate the variance and mean of each variable
    normalizer = tf.keras.layers.Normalization()

    @ tf.function
    def pick_only_data(data, _):
        # this function is just a helper to choose only data and not label
        # we could use lambda syntax, but sometimes tf has a problem converting it to a Graph Mode
        return data
    # adapt method will calculate the means and variances, and set them accordinigly
    # again we use the mapping to pick just data with predefind function
    ...
    # we utilize the so called 'functional API'
    ...
    # using the fucntional API it is easy to create multiple hidden layers in a sequance in a loop
    for layer_size in args.hidden_layers:
        ...
        # in our case, where we have a lot of data, over fitting is not an issue, so it is not neccesery
        # you can try to play with it, for example to train for 100 epochs and after seeing overfitting, raising the value of dropout to 0.5
        hidden = tf.keras.layers.Dropout(args.dropout)(hidden)
    # output layer is just a one number between 0 and 1, thats why we use the sigmoid=1/(1+exp(-x)) activation
    output = tf.keras.layers.Dense(1, activation=tf.nn.sigmoid)(hidden)
    # here we create the model with input and output
    model = tf.keras.Model(inputs=input, outputs=output)
    # we need to compile the model, ie to specify the loss used for minimization,
    # optimizer used to calculate the changes to the trainable variables
    # metrics we want to monitor, in our case the accuracy and AUC (of ROC curve)
